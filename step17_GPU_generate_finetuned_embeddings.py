"""
Step 17 — Generate fine-tuned image embeddings for the full dataset - GPU Setup 
...
# --------------------------------------------------
# 5) Check required files and load the full metadata
# --------------------------------------------------
# This file gives us:
...
for p in [INPUT_CSV, CHECKPOINT_PATH]:
    if not p.exists():
...
if "image_path" not in df.columns:
    raise ValueError("Column 'image_path' not found in input metadata.")

# Apply optional image-path remapping
df["image_path_runtime"] = df["image_path"].astype(str).apply(resolve_image_path)

# Quick safety check before the long loop
missing_sample = sum(not Path(p).exists() for p in df["image_path_runtime"].head(20))
print("Device:", DEVICE)
print("Metadata rows:", len(df))
print("Checkpoint path:", CHECKPOINT_PATH)
print("Output embedding file:", OUTPUT_EMB)
print("Output metadata file:", OUTPUT_META)
print("Missing among first 20 runtime image paths:", missing_sample)

if missing_sample > 0:
    raise FileNotFoundError("Some runtime image paths do not exist. Check path remapping or metadata.")
